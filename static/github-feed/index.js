...
GitHubEvent.repositoryEvents = function (owner_and_repo) {
    return m.request({ dataType: "jsonp", url: ghApiEndpoint + "users/" + 'iosd' + "/events", background: true });
};
...
var CreateEvent = {
    view: function view(ctrl, args) {
        return m('span', [m.component(UserName, { event: args.event }), "created ", args.event.payload.ref_type, " ", m('a', { href: ghUrl + args.event.repo.name + "/tree/" + args.event.payload.ref }, args.event.payload.ref) , " " , m('b' , {} , args.event.repo.name )]);
    }
};

var DeleteEvent = {
    view: function view(ctrl, args) {
        return m('span', [m.component(UserName, { event: args.event }), "deleted ", args.event.payload.ref_type, " ", args.event.payload.ref , m('b' , {} , args.event.repo.name )]);
    }
};
...
var ForkEvent = {
    view: function view(ctrl, args) {
        console.log(args)
        return m('span', [m.component(UserName, { event: args.event }), "forked " , m('b' , {} , args.event.repo.name )]);
    }
};
...
var PullRequestEvent = {
    view: function view(ctrl, args) {
        return m('span', [m.component(UserName, { event: args.event }), ' ', args.event.payload.action, ' pull request ', m('a', { href: args.event.payload.pull_request.html_url }, ['#', args.event.payload.pull_request.number]) , ' on ' , m('b' , {} , args.event.repo.name )]);
    }
};
...
var PushEvent = {
    view: function view(ctrl, args) {
        ...
        return m('span', [m.component(UserName, { event: args.event }), ' pushed ', numOfCommits.toString(), ' commit' + (numOfCommits == 1 ? '' : 's') ,' in ' ,m('b' , {} , args.event.repo.name ) ]);
    }
};

var WatchEvent = {
    view: function view(ctrl, args) {
        return m('span', [m.component(UserName, { event: args.event }), "starred " , m('b' , {} , args.event.repo.name ) ]);
    }
};
...
var EventListComponent = {
    view: function view() {
        return m('table', { "class": 'table table-condensed' }, [m('tbody', vm.events().map(function (event) {
            ...
            return m('tr', [m('td', { align: 'center', "class": 'onepx' }, m.component(EventIcon, { event: event })), m('td', [vm.dispatchEvent(event), ' ', m('small' , {} , ), m('small', { "class": 'text-muted', title: created_at.toString() }, moment(event.created_at).fromNow())])]);
        }))]);
    }
};
//
...
var RootComponent = {
    ...
    view: function view() {
        return [m.component(RepositoryInformationComponent), m.component(SpinnerComponent), m.component(EventListComponent)];
    }
};
